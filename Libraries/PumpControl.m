...
function Start(block)
  ...
  fopen(sp);
  fwrite(sp, rtuwritecoil(DEV_ADDRESS, 16, 04, 65280)); % start RS485 control

  async_queue_timer = timer;
  async_queue_timer.Period = 0.1;
...
function Update(block)
  ...
  if block.Dwork(ind).Data ~= block.InputPort(ind).Data
      block.Dwork(ind).Data = block.InputPort(ind).Data;
      fprintf('Pump direction updated: %d\n', block.Dwork(ind).Data);
...
userdata = get(block.BlockHandle,'UserData');
sp = userdata.sp;
fwrite(sp, rtuwritecoil(DEV_ADDRESS, 16, 04, 0)); % stop RS485 control
fclose(userdata.sp);
end %endfunction
